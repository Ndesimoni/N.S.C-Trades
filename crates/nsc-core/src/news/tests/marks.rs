...
//!
//! His ask, 1 September 2026: *"five minutes to the news, the card has to pop
//! up again."* So `warn_at_minutes = [30, 5]` — a heads-up half an hour out
//! and a last call five minutes out.
//!
...
fn nothing_is_live_before_the_widest_mark() {
    assert_eq!(at_minutes_before(31), None, "still too far off");
}

#[test]
fn the_half_hour_mark_owns_the_stretch_down_to_five() {
    assert_eq!(at_minutes_before(30), Some(30), "the moment it opens");
    assert_eq!(at_minutes_before(20), Some(30));
    assert_eq!(at_minutes_before(6), Some(30), "the last minute it owns");
}

/// **The handover, and the one that would break first.**
///
/// At exactly five minutes out both marks would qualify if the windows simply
/// ran from their own start to the event. The narrower one wins, because it is
/// the newer piece of news.
#[test]
fn the_five_minute_mark_takes_over_at_five() {
    assert_eq!(at_minutes_before(5), Some(5), "not 30 — the narrower wins");
    assert_eq!(at_minutes_before(1), Some(5));
}

#[test]
fn the_last_mark_is_the_one_that_outlives_the_release() {
    let printed = event("Core PCE", nine(), Impact::High);
    assert_eq!(due_at(&printed, nine(), &rules()), Some(5), "as it prints");
    ...
    assert_eq!(due_at(&gone, nine(), &rules()), Some(5));
...
///
/// Five minutes to a release is news he can act on. Half an hour ago is
/// history, and a card headed "in 30 minutes" about something that has already
/// printed is worse than silence.
#[test]
fn coming_back_up_after_it_printed_gives_the_five() {
    let just_gone = event("Core PCE", nine() - Duration::minutes(2), Impact::High);

    assert_eq!(due_at(&just_gone, nine(), &rules()), Some(5));
}
...
fn the_order_they_are_written_in_does_not_matter() {
    ...
    muddled.warn_at_minutes = vec![5, 30, 30];

    let soon = event("Core PCE", nine() + Duration::minutes(20), Impact::High);
    assert_eq!(due_at(&soon, nine(), &muddled), Some(30));

    let closer = event("Core PCE", nine() + Duration::minutes(3), Impact::High);
    assert_eq!(due_at(&closer, nine(), &muddled), Some(5));
}
